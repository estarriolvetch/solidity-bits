...
const BitMap = artifacts.require('BitMapMock2');
...
describe('BitMap', function (accounts) {
  ...
  describe('popcount', function () {
    ...
    it('within a single bucket', async function () {
      ...
      assert.equal(await this.bitmap.popcount(1,150), 150);
      assert.equal(await this.bitmap.popcount(0,256), 150);
      assert.equal(await this.bitmap.popcount(151,256), 0);
    ...
    it('across multiple bucket', async function () {
      ...
      assert.equal(await this.bitmap.popcount(0,600), 600-512);
      assert.equal(await this.bitmap.popcount(10,512), 0);
